...
**Hot path**: code that runs at high frequency (for example every game tick). Hot paths must follow strict allocation and timing rules. See [Hot-path discipline](Hot-Path-Discipline).
...
**packageId prefix**: every registered section or metric name is automatically prefixed with the calling mod's RimWorld `packageId` (PRD 35.69). For example, mod `yourmod.example` registering `"tick"` produces the wire name `yourmod.example.tick`. See [Profile API](Profile-API).
...
**Session**: one RimWorld game launch. The collector stores each session as a separate row in its SQLite database. See [Using the collector](Using-The-Collector).
