...
Common problems and how to fix them. Written for players and mod authors.
...
### A section does not appear in the dashboard

**Cause:** Most often, your code creates the section handle inside a hot loop, such as a `Tick` method or a Harmony postfix. It should be a static field that you create once at startup. A new handle created on every call is never registered in the catalog. A less common cause is that the dashboard sidebar filter is set to a different mod's package ID, hiding your sections.
...
**Cause:** A section declared in `profiling.xml` refers to a method the patcher could not resolve. The method name, type name, or assembly reference does not match what is loaded.
...
### Cardinality limit exceeded, with values bucketed under an overflow label

**Cause:** Your code records a metric with a label value that varies unboundedly at runtime. A pawn ID, a GUID, or a tile coordinate, for example. Once distinct label combinations reach the `cardinalityLimit` (default: 100), the collector merges new combinations into an overflow bucket. It counts incidents in memory and reports them through `Diagnostics.CardinalityIncidentsTotal` and `Diagnostics.GetMetricsWithIncidents()`. It writes no `Player.log` line per incident.

**Fix:** Aggregate the label value before recording, use a category string such as `"Colonist"` or `"Animal"` instead of a specific pawn ID. If high cardinality is needed, pass a higher `cardinalityLimit` to `Obs.RegisterCounter` / `RegisterGauge` / `RegisterHistogram`. See [Metrics API](Metrics-API).
...
**Fix:**
1. Confirm the mod is active and loads before any dependents.
...
**Cause:** Heavy instrumentation on a method that runs thousands of times per tick. The most common culprit is a `profiling.xml` entry, or a code-registered section, that wraps an inner loop body rather than an outer method. That multiplies the instrumentation overhead by the loop iteration count.
...
### Workshop says RimObs is required when it is already installed
...
**Fix:** Move RimObs before all mods that depend on it in the mod list. If the error persists, verify that the `packageId` in your mod's `About.xml` matches `RimWorks.RimObs` exactly (case-insensitive, but the dot notation must be correct).
