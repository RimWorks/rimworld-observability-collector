...
In standalone mode the port is always `17654`. When the game starts the collector, the library picks the first free port at or above `25950`. It writes that port to a discovery file and passes it to the collector with `--port`. The library writes the discovery files to:
