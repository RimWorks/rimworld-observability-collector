...
public sealed class StubControlServer : System.IDisposable {
    ...
    public System.Func<ControlAssembliesResponse>? OnAssemblies { get; set; }
    ...
    private void Handle(HttpListenerContext ctx) {
        ...
        if (method == "GET" && path == "/assemblies") {
            Write(ctx, WireCodec.Serialize(OnAssemblies?.Invoke() ?? new ControlAssembliesResponse()));
            return;
        }
